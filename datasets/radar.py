...
from util.box_ops import box_cxcyczwhd_to_xyzxyz


class RadarDataset(Dataset):
    def __init__(self, radar_dir, ann_file):
        self.target_processing = TargetProcessing(radar_imaging_size=(3, 3, 2))
...
class TargetProcessing:
    def __init__(self, radar_imaging_size):
        self.h, self.w, self.z = radar_imaging_size
        self.nx, self.ny, self.nz = 25, 25, 1
    ...
    def __call__(self, radar_signal, target):
        ...
        boxes_min_xyz = box_cxcyczwhd_to_xyzxyz(boxes)[:, :3].unsqueeze(1)
        ...
        reshape_keypoints = keypoints.reshape(keypoints.shape[0], -1, 3)
        displacement = (reshape_keypoints - boxes_min_xyz) / boxes[:, 3:].unsqueeze(1)
        displacement = displacement.reshape(displacement.shape[0], -1)
        classes = [objects["category_id"] + 1 for objects in annotations]
        classes = torch.tensor(classes, dtype=torch.float32)

        keep = True
        target = {}
        target["boxes"] = boxes
        target["keypoints"] = displacement
...
def build_dataset(dataset, base_dir):
    PATHS = {
        "train": (base_dir + "revised_train/subtracted_radar_all", base_dir + "revised_train/annotation_keypoint_train_scaled_all.json"),
        "val": (base_dir + "revised_test/subtracted_radar_all", base_dir + "revised_test/annotation_keypoint_test_scaled_all.json"),
        "test": (base_dir + "revised_test/subtracted_radar_all", base_dir + "revised_test/annotation_keypoint_test_scaled_all.json")
    }
