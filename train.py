...
def get_args_parser():
    ...
    parser.add_argument('--trans_activation', default='gelu', type=str)

    parser.add_argument('--loss_boxes_coef', default=1, type=float)
    parser.add_argument('--loss_keypoints_coef', default=1, type=float)
    parser.add_argument('--loss_giou_coef', default=5, type=float)
    parser.add_argument('--loss_object_coef', default=12, type=float)
    parser.add_argument('--empty_weight', default=300, type=float)

    parser.add_argument('--matcher_cost_boxes', default=1, type=float)
    parser.add_argument('--matcher_cost_keypoint', default=1, type=float)
    parser.add_argument('--matcher_cost_giou', default=1, type=float)
    parser.add_argument('--matcher_cost_obj', default=1, type=float)
...
def main(args):
    if args.wandb:
        wandb.init(project='3D_TransPOSE', config=args)
        args = wandb.config
    ...
    model = RDDETR(args.num_stacked_seqs, args.d_model, args.num_queries, args.n_heads, args.num_layers,
                   args.dim_feedforward, args.trans_dropout, args.trans_activation).to(device)
    ...
    criterion = SetCriterion(args.empty_weight, matcher_weights).to(device)
    weight_dict = {'loss_keypoints': args.loss_keypoints_coef, 'loss_giou': args.loss_giou_coef,
                   'loss_boxes': args.loss_boxes_coef, 'loss_object': args.loss_object_coef}
    ...
    for epoch in range(args.epochs):
        ...
        #                               weight_dict=weight_dict, optimizer=optimizer, device=device, epoch=epoch,
        #                               max_norm=args.clip_max_norm, use_wandb=args.wandb)
        # lr_scheduler.step()
        keypoint_thresh_list = [eval(x) for x in args.val_keypoint_thresh_list.split(',')]
        val_stats = evaluate(model=model, data_loader=test_dataloader, device=device,
